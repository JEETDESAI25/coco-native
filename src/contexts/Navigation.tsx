...
export const NavigatorContext: React.Context<ScreenNavigator> =
  React.createContext({
...
function NavigationTab(props: NavigationTabProps): JSX.Element {
  ...
  Keyboard.addListener(
    'keyboardWillShow',
    updateKeyboardIsVisible.bind(null, true),
  );
  Keyboard.addListener(
    'keyboardWillHide',
    updateKeyboardIsVisible.bind(null, false),
  );
  Keyboard.addListener(
    'keyboardDidShow',
    updateKeyboardIsVisible.bind(null, true),
        ...
        {Object.keys(tabContent).map((key, index) => (
          <NWTouchableHighlight
            ...
            <NWView className=" flex-0 flex-col items-center min-w-[20%] mt-2">
              {tabContent[key].icon}
              <NWText className=" text-sm ">{key}</NWText>
            </NWView>
          </NWTouchableHighlight>
        ))}
      </NWView>
